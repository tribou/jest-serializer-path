...
describe("serializer", () => {
  ...
  it("replaces process.cwd with <PROJECT_ROOT> when inside string", () => {
    const sut = `long string ${path.resolve(
      process.cwd(),
      "src/somewhere"
    )} path`;
    expect(sut).toMatchSnapshot();
  ...
  it("replaces process.cwd with <PROJECT_ROOT> in an Error", () => {
    const sut = new Error(
      `some error in ${path.resolve(process.cwd(), "a/path")}`
    );
    expect(sut).toMatchSnapshot();
  });
  ...
  it("supports trailing slashes in the path", () => {
    const sut =
      path.resolve(process.cwd(), "path/with/trailing/slash/") + path.sep;
    expect(sut).toMatchSnapshot();
  });
  ...
  it("replaces every instance of process.cwd in the same string", () => {
    const sut = {
      PATH: `${path.resolve(process.cwd(), "path")}:${path.resolve(
        process.cwd(),
        "another/path"
      )}`,
  ...
  it("replace all roots inside Map", () => {
    const homeDir = os.homedir();
    const tempDir = os.tmpdir();
  ...
  it("replace all roots inside Set", () => {
    const homeDir = os.homedir();
    const tempDir = os.tmpdir();

    const sut = new Set([
      "isSet",
      path.resolve(cwd, "nested/cwd"),
      path.resolve(homeDir, "nested/home"),
      path.resolve(tempDir, "nested/temp")
    ]);
...
describe("normalizePaths", () => {
  ...
  it("use <TEMP_DIR> when nested inside <HOME_DIR>", () => {
    ...
    jest.spyOn(os, "tmpdir").mockImplementation(() => tempDir);
  ...
  it("use <HOME_DIR> when nested inside <TEMP_DIR>", () => {
    ...
    jest.spyOn(os, "homedir").mockImplementation(() => homeDir);
  ...
  describe("symlinks", () => {
    ...
    it("handles process.cwd as symlink", () => {
       ...
       * is not returned
       */
      jest.spyOn(process, "cwd").mockImplementation(() => linkedCwd);
    ...
    it("handles process.cwd as symlink nested inside itself", () => {
      ...
      jest.spyOn(fs, "realpathSync").mockImplementation(() => realCwd);
    ...
    it("handles temp dir as symlink", () => {
      ...
      jest.spyOn(os, "tmpdir").mockImplementation(() => linkedTempDir);
    ...
    it("handles home dir as symlink", () => {
      ...
      jest.spyOn(os, "homedir").mockImplementation(() => linkedHomeDir);
